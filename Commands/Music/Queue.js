    ...
    async execute(client, message, args, prefix, color, dispatcher) {
        ...
        if (queuedSongs.length <= 10) {
            ...
            const collector = msg.createMessageComponentCollector({
                filter: (b) => { if (b.member.user.id === message.author.id) return true; else { b.reply({ ephemeral: true, content: `Only **${message.author.tag}** can use this button.` }); return false; } },
                time: 60000 * 5,
                idle: 30e3,
            });
