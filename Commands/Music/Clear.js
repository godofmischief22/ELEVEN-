    ...
    async execute(client, message, args, prefix, color, dispatcher) {
        ...
        const collector = m.createMessageComponentCollector({
            filter: (b) => {
                ...
                else { b.reply({ ephemeral: true, content: `Only **${message.author.tag}** can use this button, if you want then you've to run the command again.` }); return false; }
            },
            max: 1,
            time: 60000,
            idle: 60000 / 2
        ...
        collector.on('collect', async (b) => {
            if (b.customId === 'cqueue') {
                ...
                if (!dispatcher.queue) {
                    const mmmm = client.embed().setColor(color).setDescription('There Is Nothing In The Queue');
                ...
                return await m.reply({ embeds: [client.embed().setDescription('Cleared the Queue.').setColor(color)] });
            }
            if (b.customId === 'cfilter') {
                ...
                await client.util.update(dispatcher, client);
                await m.edit({ embeds: null, components: [client.row().addComponents(but.setDisabled(true), but2.setDisabled(true))] });
                return await m.reply({ embeds: [client.embed().setDescription('Cleared the Filter.').setColor(color)] });
            }
        });
    }
})
