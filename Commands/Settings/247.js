...
module.exports = new Object({
    ...
    async execute(client, message, args, prefix, color, dispatcher) {
        ...
        if (!data) {
            ...
            return await client.util.msgReply(message, `24/7 mode is now **${enable[~~(Math.random() * enable.length)]}**.`, color);
        } else if (data.mode) {
            data.mode = false;
            ...
            await data.save();
            return await client.util.msgReply(message, `24/7 mode is now **${disable[~~(Math.random() * disable.length)]}**.`, color);
        } else {
            data.mode = true;
